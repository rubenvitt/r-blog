...
Was das Soll konkret ausmacht, woraus so ein Contract besteht, damit er mehr wird als ein hübsches Wort, klären wir im nächsten Kapitel.

## Der Agent Contract: das Soll als ein versioniertes Artefakt

Kapitel 2 hat 'revisionssicher' und 'Agent Contract' als meine Engineering-Sprache markiert und die eigentliche Rechtspflicht auf ihren Kern gebracht: Unveränderbarkeit. Das beantwortet aber noch nicht, was da unveränderbar festgehalten werden soll. Was ist das Soll, dessen Einhaltung das Log später beweisen muss?
...
In ai-rollout-canary-shadow habe ich dieses Bündel schon einmal zusammengeführt, als Release-Manifest: alle Artefakte mit Hashes und Pins an einem Git-Tag, damit ein Rollback atomar wird. Hier gebe ich dem Bündel einen Namen und eine Rolle. Ich nenne es den Agent Contract. Das ist mein Begriff aus der Engineering-Praxis, kein Rechtsbegriff - in DORA, den RTS, der GoBD oder dem AI Act steht er nirgends. Für mich beschreibt er das vollständige Soll eines AI-Features in einem versionierten Artefakt.
...
```yaml
contract: bonitaets-agent@2026.06.30
output_schema:
  ref: schemas/bonitaet-v4.json
  sha256: 7c4a9e...
prompt:
  version: prompts/bonitaet-v11
  sha256: 3a1f7d...
allowed_tools:
  manifest: tools.pinned.json          # nur schufa_lookup, income_verify
  sha256: 9e8b21...
model:
  provider: anthropic
  id: claude-sonnet-4-7                 # datierter Snapshot, kein -latest
  pinned_at: 2026-06-15
eval_baseline:
  set: evals/bonitaet-baseline-2026-06.jsonl
  version: v7
  gate: thresholds/bonitaet-v3.yaml
signature: sigstore/bonitaets-agent-2026.06.30.bundle   # DSSE + in-toto, Rekor
approved_by:
...
<figcaption class="mt-3 text-center text-sm text-(--color-text-muted)">Prompt, Schema, Tools, Parameter, Eval-Set und selbstgehostete Gewichte lassen sich hashen und signieren. Das gehostete Modell nicht: hier bleibt nur der Versionsstring, und die Gewichte liegen beim Provider. Genau diese Lücke füllt das Log.</figcaption>
</figure>
...
Bei OpenAI läuft dieselbe Trennung unter anderen Namen: datierte Snapshots gegen das floating `-latest`. Wer `-latest` in den Contract schreibt, hat einen Alias gepinnt und nichts festgezurrt. Wenigstens bleibt Reaktionszeit, denn für GA-Modelle kündigt OpenAI eine Deprecation mindestens sechs Monate im Voraus an.
...
Kapitel 4 hat den unbequemen Teil offengelegt: Der Modellteil des Contracts lässt sich nicht signieren. Gehostete Gewichte kann niemand hashen, wer eine API anspricht, besitzt sie nicht, und der einzige Anker, der bleibt, ist der Versionsstring. Bleibt eine Frage offen: Wenn ich das Soll nicht kryptografisch an das ausgelieferte Modell binden kann - wie beweise ich dann, was tatsächlich lief?

Der Contract, wie ich ihn hier nenne (ein Engineering-Framing, kein Rechtsbegriff), beschreibt das Soll: welches Modell, welcher Prompt, welche Tools, welche Freigabe gegen welche Eval-Baseline. Das Log ist der Ist-Nachweis. Ohne das zweite ist das erste eine Behauptung. Und ein Log, das diesen Nachweis tragen soll, ist mehr als ein Trace fürs Debugging.
...
Das ist der still gewechselte Modellstring. Die Model-ID ist ein pinned snapshot, aber der Alias darüber (ein "production"-Zeiger, ein Azure-Default) ist beweglich und kann unter dir umziehen. Hätte das Log nur "production" gespeichert, wäre die Drift unsichtbar. Weil es den aufgelösten, datierten Wert pro Call festhält, fällt der Bruch zwischen Soll und Ist auf. Das ist die Caching-Staleness-Falle: Ein Label ist bequem und lügt nicht, es sagt nur nichts darüber, worauf es gestern zeigte.
...
Eine feste Aufbewahrungsfrist nennt Art. 12 bewusst nicht. Das Finanzunternehmen legt die Speicherfrist selbst fest und begründet sie risiko- und zweckbasiert. Was das für die gerichtsfeste Haltbarkeit heißt, klärt Kapitel 6.

### Was nicht ins Log gehört

Ein letzter Zug, bevor es weitergeht. Vollständigkeit der Conformance-Metadaten heißt nicht, dass alles hineingehört. Die SCHUFA-Auskunft, das Einkommen, der Name des Antragstellers - das sind genau die Daten, die im Trace nichts verloren haben. Ich habe in "Observability für AI-Features" beschrieben, was nicht in den Trace gehört; dieselbe Grenze gilt hier. Der Conformance-Nachweis braucht die `contract_version` und das aufgelöste Modell, nicht den Klartext der Kreditakte. Dieses Spannungsfeld zwischen Nachweispflicht und Datenschutz (DSGVO) löst Kapitel 7 auf.
...
Zwei Fußnoten, die über die Jahre entscheiden. Erstens ist ein Zeitstempel nicht ewig gültig. Signaturalgorithmen altern, Zertifikate laufen ab. RFC 4998 (Evidence Record Syntax) beschreibt, wie der Beweiswert über lange Aufbewahrungsfristen erhalten bleibt: durch rechtzeitige Erneuerung der Zeitstempel, bevor das alte Verfahren kryptografisch schwach wird. Bei zehn Jahren Aufbewahrung wird daraus ein Dauerprozess, den jemand aktiv betreiben muss. Zweitens gibt es in Deutschland dafür einen dokumentierten Stand der Technik: Die BSI TR-03125 (TR-ESOR) beschreibt die Architektur für beweiswerterhaltende Langzeitspeicherung. Wer sich daran hält, muss nicht selbst begründen, dass seine Aufbewahrung dem aktuellen Stand entspricht.

<Accordion title="Aufbewahrungsfristen und der BaFin-Carve-out">
Die Aufbewahrungsdauer staffelt § 257 Abs. 4 HGB: Handelsbücher, Inventare und Bilanzen zehn Jahre, Buchungsbelege acht Jahre, empfangene Handelsbriefe sechs Jahre. Satz 2 enthält den für uns entscheidenden Carve-out: Für Institute im Sinne des § 1 Abs. 1b KWG, des § 1 Abs. 1 VAG und des § 2 Abs. 1 WpIG bleiben Buchungsbelege bei zehn Jahren. Wer eine BaFin-regulierte Bank oder Versicherung baut, kalkuliert also mit zehn statt acht Jahren.

Und die Frist ist nach oben offen. § 147 Abs. 3 S. 5 AO kennt eine Ablaufhemmung: Solange die Unterlagen für eine noch nicht abgelaufene Festsetzungsfrist von Bedeutung sind, etwa in einer laufenden Betriebsprüfung, läuft die Aufbewahrungsfrist nicht ab. Ein Log, das ich für einen Kredit-Score aus 2026 führe, kann mich deutlich länger als zehn Jahre begleiten. Damit ist RFC 4998 kein akademisches Detail.
</Accordion>
...
Damit steht der Stack: append-only, Merkle-Root, qualifizierter Zeitstempel, WORM darunter. Technisch ist das die belastbarste Form, ein Kredit-Log über ein Jahrzehnt beweisbar zu halten. Bleibt die Frage, die dieses Kapitel bewusst offengelassen hat: Wer schreibt einer Bank eigentlich vor, so etwas zu führen - und warum ausgerechnet jetzt? Was davon ist heute bindend, was bloß Empfehlung, und was verschiebt sich gerade im Kalender? Das ordnet das nächste Kapitel.
...
Zur Erinnerung aus Kapitel 2: "revisionssicher" und "Agent Contract" sind mein Framing, kein Wortlaut dieser Regime - ich trage sie hier bewusst in den regulatorischen Kontext, als Übertragung, nicht als Zitat.

### DORA ist der Anker, nicht VAIT

DORA gilt seit dem 17. Januar 2025, unmittelbar in jedem Mitgliedstaat, ohne nationale Umsetzung (Art. 64). Das ist der Boden, auf dem für Finanzunternehmen alles andere steht. Aufzeichnung verlangt die Verordnung allerdings nicht dort, wo wir zuerst suchen: Der Anker sitzt eine Ebene tiefer, im technischen Regulierungsstandard Del. VO 2024/1774 (RTS RMF). Dessen Art. 12 "Datenaufzeichnung" fordert "Maßnahmen zum Schutz von Datenaufzeichnungssystemen und -informationen vor Manipulation, Löschung und unbefugtem Zugriff", und zwar für gespeicherte, übermittelte und gerade verwendete Daten. Das Schutzprofil, das unser Log technisch bereits erfüllt.

Zwei Details sind für unseren Fall zentral. Das erste ist die Uhr: Art. 12 verlangt die "Synchronisierung der Uhren jedes IKT-Systems … auf der Grundlage einer dokumentierten zuverlässigen Referenzzeitquelle", der Zeitstempel im Log ist also keine Kür. Das zweite ist die Retention: Eine feste Frist schreibt DORA nicht vor; das Finanzunternehmen legt die "Speicherfrist" risiko- und zweckbasiert selbst fest und begründet sie. Wer 6 Monate wählt, muss das begründen können, wer 10 Jahre wählt, ebenso.
...
<Callout type="warning">
Wer 2026 noch VAIT, BAIT, KAIT oder ZAIT zitiert, arbeitet mit einer veralteten Landkarte. VAIT, KAIT und ZAIT wurden aufgehoben, BAIT ist ausgelaufen. Der Anker für die IKT- und Auslagerungsanforderungen heißt jetzt DORA samt der zugehörigen RTS. Für die Modellebene kommt die MaRisk hinzu, deren KI-Modul allerdings noch in Konsultation ist.
</Callout>
...
Die Kostenfrage aus dem letzten Kapitel kommt selten allein. Sobald ich Contract und revisionssicheres Log als Pflichtprogramm auf den Tisch lege (beides mein Engineering-Framing, kein Begriff aus DORA oder dem AI Act), kommen die Einwände zuverlässig. Ein paar davon höre ich in fast jeder Diskussion. Sie sind gut genug, um sie ernst zu nehmen.

### "Revisionssicherheit ist reine Compliance-Fassade."

Der Verdacht ist berechtigt: Vieles, was unter "Compliance" läuft, ist ein Ordner, den nie jemand öffnet. Nur trifft es hier nicht zu. Das Log ist die einzige Fläche, gegen die sich der Contract überhaupt diffen lässt - der einzige Mechanismus, mit dem du einen stillen Modell-Swap, driftende Prompts, einen out-of-scope Tool-Call oder eine gebrochene Schema-Zusage überhaupt bemerkst. Ein Soll ohne Ist bleibt eine Behauptung. Denselben Einwand habe ich beim Eval-Loop schon gehört, und die Antwort ist dieselbe: Die Substanz entsteht im Betrieb, der Nachweis fällt nebenbei ab.
...
<InfoBox>
  **Primärquellen (Stand Juli 2026, Regulatorik in Bewegung):**<br />
  ...
  BaFin - Orientierungshilfe "IKT-Risiken beim Einsatz von KI" (18.12.2025): [bafin.de](https://www.bafin.de/SharedDocs/Veroeffentlichungen/DE/Meldung/2025/meldung_2025_12_18_orientierungshilfe_ikt_risiken.html)<br />
  BaFin - 9. MaRisk-Novelle, Konsultation 02/2026: [bafin.de](https://www.bafin.de/SharedDocs/Veroeffentlichungen/DE/Meldung/2026/meldung_2026_04_01_konsultation_02_2026_marisk-novelle.html)
</InfoBox>
